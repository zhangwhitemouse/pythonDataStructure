...
def draw_lanes(img, lines, color=[255, 0, 0], thickness=8):
    ...
    left_vtx = calc_lane_vertices(left_points, 1700, img.shape[0])  # 拟合点集，生成直线表达式，并计算左侧直线在图像中的两个端点的坐标
    right_vtx = calc_lane_vertices(right_points, 1700, img.shape[0])  # 拟合点集，生成直线表达式，并计算右侧直线在图像中的两个端点的坐标
...
def process_an_image(img):
    # roi_vtx = np.array([[(0, img.shape[0]), (2400, 700), (2600, 700), (img.shape[1], img.shape[0])]])#目标区域的四个点坐标，roi_vtx是一个三维的数组
    roi_vtx = np.array([[(1550-550, 3440), (2300, 1500), (2800, 1500), (4200, 3440)]])
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)  # 图像转换为灰度图
    blur_gray = cv2.GaussianBlur(gray, (blur_ksize, blur_ksize), 0, 0)  # 使用高斯模糊去噪
    # cv2.namedWindow("blur_gray", 0)
...
img = mplimg.imread("E:\Masters\pic\exp\p200.JPG")
